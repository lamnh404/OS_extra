...
install_manifest() {
    ...
    echo -e "${GREEN}If you find this testcase helpful, please give me a star on GitHub ❤ ${NC}"
    exit 0
}
...
echo -e "Output files ${RED}(OUTPUT_/os_an*.txt)${NC} will be used for result verification."
...
if ./os os_an_01 > "$TC_DIR/os_an_01.txt" 2>&1 && \
    grep -Fxq "PID=1 - Region=1 - Address=00000000 - Size=100 byte" "$TC_DIR/os_an_01.txt" ; then
    print_result 0 "Success"
else
    print_result 1 "Error"
